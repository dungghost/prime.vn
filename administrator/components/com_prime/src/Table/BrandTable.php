 ...
 * @since 1.0.0
 */
class BrandTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_prime.brand';
		parent::__construct('#__prime_brands', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (empty($array['alias']))
		{
			...
			else
			{
				...
				{
					$array['alias'] = OutputFilter::stringURLUnicodeSlug(trim($array['brand']));
				}
				else
				{
					$array['alias'] = OutputFilter::stringURLSafe(trim($array['brand']));
				}
			}
		}
		...
		if (!$user->authorise('core.admin', 'com_prime.brand.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_prime/access.xml',
				"/access/section[@name='brand']/"
			);
			$default_actions = Access::getAssetRules('com_prime.brand.' . $array['id'])->getData();
			$array_jaccess   = array();

 ...
 * @since 1.0.0
 */
class SurfaceTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_prime.surface';
		parent::__construct('#__prime_surfaces', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (empty($array['alias']))
		{
			...
			else
			{
				...
				{
					$array['alias'] = OutputFilter::stringURLUnicodeSlug(trim($array['surface']));
				}
				else
				{
					$array['alias'] = OutputFilter::stringURLSafe(trim($array['surface']));
				}
			}
		}
		...
		if (!$user->authorise('core.admin', 'com_prime.surface.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_prime/access.xml',
				"/access/section[@name='surface']/"
			);
			$default_actions = Access::getAssetRules('com_prime.surface.' . $array['id'])->getData();
			$array_jaccess   = array();

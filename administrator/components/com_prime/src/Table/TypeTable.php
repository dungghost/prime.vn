 ...
 * @since 1.0.0
 */
class TypeTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_prime.type';
		parent::__construct('#__prime_types', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (empty($array['alias']))
		{
			...
			else
			{
				...
				{
					$array['alias'] = OutputFilter::stringURLUnicodeSlug(trim($array['type']));
				}
				else
				{
					$array['alias'] = OutputFilter::stringURLSafe(trim($array['type']));
				}
			}
		}
		...
		if (!$user->authorise('core.admin', 'com_prime.type.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_prime/access.xml',
				"/access/section[@name='type']/"
			);
			$default_actions = Access::getAssetRules('com_prime.type.' . $array['id'])->getData();
			$array_jaccess   = array();

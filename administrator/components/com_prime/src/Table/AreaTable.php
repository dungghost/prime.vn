 ...
 * @since 1.0.0
 */
class AreaTable extends Table implements VersionableTableInterface, TaggableTableInterface
{
	...
	{
		$this->typeAlias = 'com_prime.area';
		parent::__construct('#__prime_areas', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (empty($array['alias']))
		{
			...
			else
			{
				...
				{
					$array['alias'] = OutputFilter::stringURLUnicodeSlug(trim($array['area']));
				}
				else
				{
					$array['alias'] = OutputFilter::stringURLSafe(trim($array['area']));
				}
			}
		}
		...
		if (!$user->authorise('core.admin', 'com_prime.area.' . $array['id']))
		{
			$actions         = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_prime/access.xml',
				"/access/section[@name='area']/"
			);
			$default_actions = Access::getAssetRules('com_prime.area.' . $array['id'])->getData();
			$array_jaccess   = array();
